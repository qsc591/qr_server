...
cd "$APP_DIR"
if [[ ! -d "wechat_qr_server" || ! -d "wechat_qr_board" || ! -d "login" ]]; then
  echo "ERROR: expected wechat_qr_server/, wechat_qr_board/, login/ under $APP_DIR" >&2
  exit 1
fi
if [[ ! -f "requirements.txt" ]]; then
  echo "ERROR: missing requirements.txt under $APP_DIR" >&2
  exit 1
fi
python3 -m venv .venv
...
cat > wechat_qr_server/config.json <<EOF
{
  ...
  "kakao_group_enabled": true,
  "countdown_seconds": 415,
  "pgw_client_id": "6940c10b-6860-4233-be3e-b935c44d9fbb",
  "seat_field_name_patterns": ["seat info", "seat", "位置", "座位"],
  "account_field_name_patterns": ["account", "账号", "login", "id", "password", "pass"],
  "web": {
    "host": "127.0.0.1",
    "port": ${PORT},
...
echo ""
echo "Note (optional): If you enable TTM QR capture, install Chromium for Playwright:"
echo "  source ${APP_DIR}/.venv/bin/activate && python -m playwright install chromium"
echo ""
